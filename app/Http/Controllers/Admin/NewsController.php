...
class NewsController extends Controller
{
    ...
    protected $viewRoot = "admin.pages.news.";
    ...
    public function index()
    {
        ...
        $data['news'] = $this->model->latest()->get();
    ...
    public function store(Request $request)
    {
        try {
            ...
            if($request->file('image')){
                ...
                Storage::disk('public')->put('uploads/news/'.$fileName, $document->getContent());
    ...
    private function slugGenerate($id = null){
        ...
        $counts = News::where('slug',$slug)->when($id,function($q) use($id) {
            $q->where('id','<>', $id);
        })->count();
        if(!empty($counts)){
            $slug += "-".$counts;
    ...
    public function edit(string $id)
    {
        ...
        $data['news'] = $this->model->findOrFail($id);
        $data['projects'] = Project::select('id','title')->get();
    ...
    public function update(Request $request, string $id)
    {
        try {
            ...
            if($request->file('image')){
                ...
                $fillableData['image'] = $fileName;
                if($model->image){
                    if(Storage::exists('uploads/news/'.$model->image)){
                        Storage::delete('uploads/news/'.$model->image);
                    }
                }
                Storage::disk('public')->put('uploads/news/'.$fileName, $document->getContent());
            }
    ...
    public function destroy(Project $project)
    {
        try {
            if($project->image){
                if(Storage::exists('uploads/news/'.$project->image)){
                    Storage::delete('uploads/news/'.$project->image);
                }
            }
            $project->delete();
