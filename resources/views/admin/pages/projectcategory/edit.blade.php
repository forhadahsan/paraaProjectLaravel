  ...
  <div class="row">
    <div class="col-xl">
      <div class="card mb-4">
        ...
        <div class="card-body">
          <form action="{{ route('projectcategory.update', $projectcategory->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="row mb-3">
              ...
              <div class="col-sm-6">
                ...
                <input type="text" name="name" class="form-control" value="{{ $projectcategory->name }}" placeholder="Enter Name" />
              </div>

              <div class="col-sm-6">
                <label class="form-label" for="basic-default-fullname">Status</label>
                <select name="status" class="form-control" required>
                  <option value="1" {{ $projectcategory->status == 1? 'selected' : '' }}>Publish</option>
                  <option value="0" {{ $projectcategory->status == 0? 'selected' : '' }}>Not Publish</option>
                </select>
              </div>
