        ...
        <div class="card-body">
            <form action="{{ route('projects.update', $project->id) }}" method="POST" enctype="multipart/form-data">
              ...
              <div class="row mb-3">
                <div class="col-sm-6 required">
                  ...
                  <select name="category" class="form-select form-control" aria-label="Default select example" required>
                    <option value="fashion" {{ $project->category == 'fashion' ? 'selected' : ''  }}>Fashion</option>
                    <option value="design" {{ $project->category == 'design' ? 'selected' : ''  }}>Design </option>
                  </select>
                </div>
                <div class="col-sm-6">
                  <label class="form-label" for="basic-default-fullname">title</label>
                  <input type="text" name="title" class="form-control" value="{{ old('title', $project->title) }}" placeholder="Enter title" required />
                </div>
                ...
                <div class="col-sm-6">
                  ...
                  <select  name="column_type" class="form-select form-control" aria-label="Default select example" required>
                    <option value="single" {{ $project->column_type == 'single' ? 'selected' : ''  }}>Single</option>
                    <option value="double" {{ $project->column_type == 'double' ? 'selected' : ''  }}>Double</option>
                  </select>
                </div>
                <div class="col-sm-6">
              ...
              <div class="col-sm-12">
                ...
                <textarea class="form-control summernote" name="short_description">{!! $project->short_description !!}</textarea>
              </div>
            </div>
            ...
            <div class="mb-3">
              <label for="">Image</label>
                @if($project->image)
                <br>
                <img src="{{ url('storage/uploads/projects/'.$project->image) }}" width="60" />
                <br>
                @endif
