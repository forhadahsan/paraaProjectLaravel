...
<div class="container-xxl flex-grow-1 container-p-y">
  ...
  <div class="card">
    <h5 class="card-header">{{$title}}
      <a class="btn btn-primary" href="{{ route('teams.create') }}" style="float: right;">  Add <i class="bx bx-plus me-1"></i></a>
    </h5>
        ...
        <table class="table" id="data-table">
          ...
          <tbody class="rable table-border-bottom-0">
            ...
            <tr>
             ...
             <td>
                @if($value->image)
                <img src="{{ url('storage/uploads/teams/'.$value->image) }}" width="60" />
                @else
                ---
                @endif
            </td>
            ...
            <td>
                <div class="dropdown">
                  ...
                  <div class="dropdown-menu">
                    <a class="dropdown-item"  href="{{ route('teams.edit', $value->id) }}"><i class="bx bx-edit-alt me-1"></i> Edit</a>
                    <a class="dropdown-item" onclick="return deleteData({{$value->id}})" href="javascript:void(0);"><i class="bx bx-trash me-1"></i> Delete</a>
                  </div>
                </div>
                <form action="{{ route('teams.destroy',$value->id) }}" method="POST" id="delete-form-{{$value->id}}">
                  @method("DELETE")
                  @csrf
                </form>
              </td>
